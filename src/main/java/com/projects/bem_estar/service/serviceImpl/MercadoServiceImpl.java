...
import com.projects.bem_estar.service.serviceImpl.EnderecoServiceImpl;
...
public class MercadoServiceImpl implements MercadoService {
    ...
    public Mercado createMercado(Mercado mercado) {
        Endereco enderecoExistente = enderecoService.getById(mercado.getEndereco().getIdEndereco());
        if (enderecoExistente != null) {
            // Crie o Mercado e associe-o ao Endereco recuperado
            return mercadoRepository.save
                    (new Mercado(
                            mercado.getName(), mercado.getCnpj(), mercado.getEmail(), enderecoExistente, mercado.getNum_telefone(), mercado.getSenha()
                    ));
        } else {
    ...
    public Mercado updateMercado(Long id, Mercado mercado) {
        ...
        if (existingMercado != null) {
            ...
            existingMercado.setSenha(mercado.getSenha());
